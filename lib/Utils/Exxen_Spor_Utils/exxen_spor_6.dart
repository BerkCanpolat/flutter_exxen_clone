import 'package:flutter/cupertino.dart';
import 'package:flutter/material.dart';
import 'package:flutter/src/widgets/container.dart';
import 'package:flutter/src/widgets/framework.dart';
import 'package:flutter_exxen_clone/Utils/exxen_list_class.dart';

class Exxen_Spor_6 extends StatefulWidget {
  const Exxen_Spor_6({super.key});

  @override
  State<Exxen_Spor_6> createState() => _Exxen_Spor_6State();
}

class _Exxen_Spor_6State extends State<Exxen_Spor_6> {
  Future<List<Exxenspor2>> exxenSporL() async {
    var exxenlistSpor = <Exxenspor2>[];

    var exs1 = Exxenspor2(
        "kupa1.webp",
        "Shaktar Donetsk - Leipzig",
        "UEFA Şampiyonlar Ligi - 2022/23",
        "Şampiyonlar Ligi | F Grubu | 6.Hafta",
        "02.11.2022 - 20:45");
    var exs2 = Exxenspor2(
        "kupa2.webp",
        "Shaktar Donetsk - Leipzig",
        "UEFA Avrupa Ligi - 2022/23",
        "Şampiyonlar Ligi | F Grubu | 6.Hafta",
        "02.11.2022 - 20:45");
    var exs3 = Exxenspor2(
        "kupa3.webp",
        "Shaktar Donetsk - Leipzig",
        "UEFA Avrupa Konferans Ligi - 2022/23",
        "Şampiyonlar Ligi | F Grubu | 6.Hafta",
        "02.11.2022 - 20:45");
    var exs4 = Exxenspor2(
        "kupa4.webp",
        "Shaktar Donetsk - Leipzig",
        "UEFA Süper Kupası",
        "Şampiyonlar Ligi | F Grubu | 6.Hafta",
        "02.11.2022 - 20:45");
    var exs5 = Exxenspor2(
        "kupa5webp.webp",
        "Shaktar Donetsk - Leipzig",
        "UEFA Gençlik Ligi",
        "Şampiyonlar Ligi | F Grubu | 6.Hafta",
        "02.11.2022 - 20:45");

    exxenlistSpor.add(exs1);
    exxenlistSpor.add(exs2);
    exxenlistSpor.add(exs3);
    exxenlistSpor.add(exs4);
    exxenlistSpor.add(exs5);

    return exxenlistSpor;
  }

  @override
  Widget build(BuildContext context) {
    return SizedBox(
      height: 250,
      child: FutureBuilder(
          future: exxenSporL(),
          builder: (context, snapshot) {
            if (snapshot.hasData) {
              var exxenlistSpor = snapshot.data;

              return ListView.builder(
                  scrollDirection: Axis.horizontal,
                  itemCount: exxenlistSpor!.length,
                  itemBuilder: (context, index) {
                    var exS2 = exxenlistSpor[index];

                    return Column(
                      children: [
                        Padding(
                          padding: const EdgeInsets.all(6.0),
                          child: Container(
                            height: 150,
                            width: 300,
                            decoration: BoxDecoration(
                              image: DecorationImage(
                                image: AssetImage(
                                    "assets2/${exS2.exxen_spor2_resim}"),
                                fit: BoxFit.cover,
                              ),
                              borderRadius: BorderRadius.circular(12),
                            ),
                          ),
                        ),
                        Padding(
                          padding: const EdgeInsets.only(top: 10),
                          child: Container(
                            width: 295,
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(
                                  exS2.exxen_spor2_kategori,
                                  style: TextStyle(
                                    color: Colors.white,
                                    fontWeight: FontWeight.bold,
                                    fontSize: 14,
                                  ),
                                ),
                              ],
                            ),
                          ),
                        )
                      ],
                    );
                  });
            } else {
              return Center();
            }
          }),
    );
  }
}
